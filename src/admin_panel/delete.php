...
<div class="flex">
  ...
  <div> 
    ...
    if(isset($_GET['vid']) && isset($_GET['thumb'])){
      $id = $_GET['vid'];
      $image_name = $_GET['thumb'];
      ...
      <div class="flex">
        ...
        <div>
          <p class="font-bold">Something went Wrong</p>
          <p class="text-sm">Error Deleting thumbnail. please try again.</p>
        </div>
      </div>
    </div>
       ';
      ...
      //3. Delete item from Database
      $sql = "DELETE FROM tbl_vlogs WHERE id=$id";
      //Execute the Query
      $res = mysqli_query($conn, $sql);
      //CHeck whether the query executed or not and set the session message respectively
      //4. Redirect to Manage item with Session Message
...
<div> 
  <p class="font-bold">SUCCESS!</p>
  <p class="text-sm">video  Successfully Deleted.</p>
</div>
</div>
</div>
      ';
      ...
      exit();
         ...
         <div class="flex">
           ...
           <div>
             <p class="font-bold">Something went Wrong</p>
             <p class="text-sm">Error Deleting video. please try again.</p>
           </div>
         </div>
       </div>
          ';
